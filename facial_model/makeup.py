...
class makeup(object):
    ...
    def __draw_liner(self, eye, kind):
        ...
        # image = np.asanyarray(self.im_copy)
        cv2.fillPoly(self.im_copy, [points], (0, 0, 0))
        return
    ...
    def apply_eyebrow(self, landmark):
        ...
        # rescale
        right_eye = imageio.imread('./right_eyebrow.png')
        ...
        # rescale
        left_eye = imageio.imread('./left_eyebrow.png')
    ...
    def __ext(self, a, b, i):
        ...
        if b-a == 1:
            self.y.extend(int((ones(b-a)*i)).tolist())
        else:
    ...
    def apply_eyeshadow(self, landmarks_array, R, G, B):
        ...
        # Blur Filter
        b_filter = np.zeros((height, width))
        cv2.fillConvexPoly(b_filter,np.array(c_[self.yleft, self.xleft], dtype = 'int32'),1)
        cv2.fillConvexPoly(b_filter,np.array(c_[self.yright, self.xright], dtype = 'int32'),1)
        b_filter = cv2.GaussianBlur(b_filter,(31,31),0)

        # Erosion to reduce blur size
        kernel = np.ones((12, 12), np.uint8)
        b_filter = cv2.erode(b_filter, kernel, iterations = 1)
        alpha=np.zeros([height, width, 3],dtype='float64')
        alpha[:,:,0], alpha[:,:,1], alpha[:,:,2] = b_filter, b_filter, b_filter
    ...
    def apply_makeup(self, landmarks):
        # try:
            ...
            self.im_copy = self.apply_lipstick(landmarks, 0, 0, 210)
            # self.im_copy = self.apply_foundation(landmarks)
            # self.im_copy = self.apply_blush(landmarks, 223., 91., 111.)
            self.im_copy = self.apply_blush(landmarks, 111., 91., 223.)
